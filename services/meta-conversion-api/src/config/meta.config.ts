...
// Per-org override shape, persisted as JSONB on ext.meta_org_config.field_mappings.
// Any category/key an org omits falls back to DEFAULT_FIELD_MAPPINGS below.
...
// Default field-key candidates used when an org has not configured custom
// field_mappings on ext.meta_org_config. Each db column maps to an ordered
...
/**
 * Merge an org's stored field_mappings (from ext.meta_org_config.field_mappings)
 * over the hardcoded defaults. Any category/key the org has not overridden
