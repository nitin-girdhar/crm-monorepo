...
export async function lookupsRoutes(app: FastifyInstance): Promise<void> {
  app.get('/lookups', async (_request, reply) => {
    const [sources, platforms, crm.interaction_types, stages, marketing.campaign_statuses] = await withServiceTx(async (tx) => {
      return Promise.all([
        tx.unsafe(`SELECT id, name FROM crm.lead_sources ORDER BY name`),
        tx.unsafe(`SELECT id, name, description FROM marketing.marketing_platforms ORDER BY name`),
        tx.unsafe(`SELECT id, name, description FROM crm.interaction_types ORDER BY name`),
        tx.unsafe(`SELECT id, name, label, description, sort_order, followup_required, is_rejected, is_terminated FROM crm.lead_stage ORDER BY sort_order`),
        tx.unsafe(`SELECT id, name, description FROM marketing.campaign_statuses ORDER BY name`),
      ]);
    });
    return reply.status(200).send({ sources, platforms, crm.interaction_types, stages, campaign_statuses });
  });
  ...
  app.get('/lookups/cities', async (request, reply) => {
    ...
    const cities = await withServiceTx(async (tx) => {
      if (state_id !== undefined) {
        return tx.unsafe(
          `SELECT id, name FROM geo.cities WHERE state_id = $1 ORDER BY name`,
          [state_id],
        );
      }
      return tx.unsafe(`SELECT id, name, state_id FROM geo.cities ORDER BY name LIMIT 500`);
    });
    return reply.status(200).send(cities);
  });

  // Geographic lookup: countries → geo.states → geo.cities
  // Supports ?level=geo.countries|geo.states|geo.cities with optional countryIds=1,2 / stateIds=3,4
    ...
    const data = await withServiceTx(async (tx) => {
      if (level === 'geo.states') {
        if (countryIds.length) {
          return tx.unsafe(
            `SELECT id, name, code AS "isoCode", country_id AS "countryId" FROM geo.states WHERE country_id = ANY($1::int[]) ORDER BY name`,
            [countryIds],
          );
        }
        return tx.unsafe(`SELECT id, name, code AS "isoCode", country_id AS "countryId" FROM geo.states ORDER BY name`);
      }
      if (level === 'geo.cities') {
        if (stateIds.length) {
          return tx.unsafe(
            `SELECT id, name, state_id AS "stateId" FROM geo.cities WHERE state_id = ANY($1::int[]) ORDER BY name`,
            [stateIds],
          );
        }
        return tx.unsafe(`SELECT id, name, state_id AS "stateId" FROM geo.cities ORDER BY name LIMIT 500`);
      }
      // Default: return geo.countries list
      return tx.unsafe(`SELECT id, name, iso_code AS "isoCode" FROM geo.countries ORDER BY name`);
    });
    ...
    const result = await withServiceTx(async (tx) => {
      ...
      if (body['source']) {
        const src_rows = await tx.unsafe(
          `SELECT id FROM crm.lead_sources WHERE name = $1 LIMIT 1`,
          [String(body['source'])],
        );
  ...
  app.post('/cron/mark-missed-followups', async (request, reply) => {
    ...
    const result = await withServiceTx(async (tx) => {
      const missed_status_rows = await tx.unsafe(
        `SELECT id FROM crm.follow_up_statuses WHERE name = 'missed' LIMIT 1`,
      );
      const missed_id = (missed_status_rows as unknown as Array<{ id: string }>)[0]?.id;
      if (!missed_id) throw new Error('crm.follow_up_statuses: missed not found');

      const pending_status_rows = await tx.unsafe(
        `SELECT id FROM crm.follow_up_statuses WHERE name = 'pending' LIMIT 1`,
      );
      const pending_id = (pending_status_rows as unknown as Array<{ id: string }>)[0]?.id;
      if (!pending_id) throw new Error('crm.follow_up_statuses: pending not found');
