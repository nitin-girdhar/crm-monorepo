...
export async function listUsers(
  ...
  return withRoleTx(ctx, async (tx) => {
    // Join via user_org_mapping so multi-org users (whose home org differs)
...
export async function getUserById(
  ...
  const ctx: RoleTxContext = { role, org_id, tenant_id, user_id: actor_user_id };
  return withRoleTx(ctx, async (tx) => {
    // Join user_org_mapping to get the org-specific role; RLS enforces visibility.
    const rows = await tx.unsafe(
      `SELECT u.id, u.org_id, u.first_name, u.middle_name, u.last_name, u.full_name,
              u.email, u.mobile, u.is_active, u.force_password_change,
              u.password_changed_at, u.last_login_at, u.manager_id, u.created_at, u.updated_at,
              ...
              m.full_name AS manager_name
       FROM users u
       JOIN user_org_mapping uom ON uom.user_id = u.id AND uom.org_id = $2 AND uom.is_active
       JOIN user_roles ur        ON ur.id = uom.role_id
       LEFT JOIN users m         ON m.id  = u.manager_id
