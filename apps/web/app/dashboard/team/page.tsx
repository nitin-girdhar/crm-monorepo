...
async function getSession(): Promise<SessionUser | null> {
  ...
  try {
    const res = await fetch(`${API_GATEWAY}/auth/me`, { headers: { cookie: cookieHeader }, cache: 'no-store' });
...
export default async function TeamPage() {
  ...
  if (!session) redirect('/login?callbackUrl=%2Fdashboard%2Fteam');
  if (session.rank < RANKS.MANAGER) redirect('/dashboard/leads');
