...
export function LeadEditModal({
  ...
  // Fetch next pending follow-up for this lead
  interface FollowUpInfo { id: string; scheduled_at: string; assigned_user_id: string; assigned_user_name: string; status_name: string }
  ...
  const fuLocalDate = nextFollowUp ? toLocalDatetime(nextFollowUp.scheduled_at) : '';

  const [editedFuDate, setEditedFuDate] = useState<string | null>(null);
  const [editedFuAssignee, setEditedFuAssignee] = useState<string | null>(null);
  const fuDateChanged = editedFuDate !== null && editedFuDate !== fuLocalDate;
  const fuAssigneeChanged = editedFuAssignee !== null && nextFollowUp && editedFuAssignee !== nextFollowUp.assigned_user_id;
  const fuFieldChanged = fuDateChanged || fuAssigneeChanged;
        ...
        <div className="flex flex-col divide-y divide-[#F1F5F9]">
          ...
          <div className="px-6 py-4">
            ...
            <div className="grid grid-cols-2 gap-x-8 gap-y-1.5">
              ...
              <InfoRow label="Follow-up" value={nextFollowUp ? new Date(nextFollowUp.scheduled_at).toLocaleString('en-IN', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : '—'} />
