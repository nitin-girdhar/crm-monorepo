...
function GroupedEventCard({ group, onUpdateFollowUp }: { group: EventGroup; onUpdateFollowUp: (fu: TimelineEvent) => void }) {
  ...
  return (
    <div className={`rounded-xl border px-4 py-3 flex flex-col gap-2 ${groupCardClass(group)}`}>
      ...
      {followUps.map((fu) => (
        <div key={fu.followupId ?? fu.eventId} className="flex flex-wrap items-center justify-between gap-1.5">
          <div className="flex flex-wrap items-center gap-1.5 text-sm font-medium text-[#0F172A]">
          ...
          {fu.followupId && ['pending', 'missed'].includes(fu.followupStatus ?? '') && (
            <button
              type="button"
...
function FollowUpActionModal({ followUp, onClose, onUpdated }: FollowUpActionModalProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setError(null);
    if (action === 'reschedule' && !scheduledAt) { setError('Please set a new date and time.'); return; }
    if (action === 'add_note' && !notes.trim())   { setError('Please enter a note.'); return; }
    if (!followUp.followupId) { setError('Follow-up ID missing — cannot update this entry.'); return; }
    ...
    try {
      ...
      const res = await fetch(
        ...
        { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body), credentials: 'include' },
      );
      if (!res.ok) {
    ...
    <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-md rounded-xl bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-[#E2E8F0] px-6 py-4">
          <h2 className="text-base font-semibold text-[#0F172A]">Update Follow-Up</h2>
          <button onClick={onClose} className="rounded-md p-1 text-[#94A3B8] hover:bg-[#F1F5F9] hover:text-[#475569]">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 px-6 py-5">
          <div>
            <label className="mb-2 block text-xs font-bold uppercase tracking-widest text-[#94A3B8]">Action</label>
            <div className="flex gap-2">
              {(['complete', 'reschedule', 'add_note'] as FollowUpAction[]).map((a) => (
                <button
                  key={a}
                  type="button"
                  onClick={() => setAction(a)}
          ...
          <div>
            ...
            <textarea
              ...
              placeholder={
                action === 'complete'    ? 'What was the outcome of this follow-up?'
                : action === 'reschedule' ? 'Why is this being rescheduled?'
...
export function LeadHistoryModal({ lead, statusLabelMap, onClose }: Props) {
  ...
  const fetchHistory = useCallback(() => {
    setLoading(true);
    fetch(`/api/leads/${lead.lead_id}/timeline`, { credentials: 'include' })
      .then(async (res) => {
        const body = await res.json() as { timeline?: TimelineEvent[]; events?: TimelineEvent[]; error?: string };
        if (!res.ok) throw new Error(body.error ?? 'Request failed');
        const evs = Array.isArray(body.timeline) ? body.timeline
    ...
    >
      ...
      >
        <div className="flex items-start justify-between border-b border-[#F1F5F9] px-6 py-4 shrink-0">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-[#64748B] truncate">{lead.full_name || '—'}</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <div className="border-b border-[#F1F5F9] px-6 py-4 shrink-0">
          ...
          <div className="grid grid-cols-2 gap-x-8 gap-y-3 sm:grid-cols-3">
            <SummaryField label="Name"        value={lead.full_name} />
            <SummaryField label="Phone"       value={lead.phone}  {...(lead.phone ? { href: `tel:${lead.phone}` } : {})} />
            <SummaryField label="Email"       value={lead.email}  {...(lead.email ? { href: `mailto:${lead.email}` } : {})} />
            ...
            <SummaryField label="Campaign"    value={lead.campaign_name} />
            <SummaryField label="Assigned To" value={lead.assigned_rep_name} />
          </div>
        </div>
                ...
                <ul className="space-y-3 pl-8">
                  {groups.map((group) => (
                    <li key={group.id} className="relative">
                      ...
                      <GroupedEventCard group={group} onUpdateFollowUp={setActiveFollowUp} />
                    </li>
                  ))}
                </ul>
              </div>
      ...
      {activeFollowUp && (
        <FollowUpActionModal
          ...
          onUpdated={() => { setActiveFollowUp(null); fetchHistory(); }}
        />
      )}
    </div>,
    document.body,
