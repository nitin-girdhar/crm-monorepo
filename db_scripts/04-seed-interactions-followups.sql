...
--            seed-02-tenants-orgs-users.sql, seed-03-leads-bulk.sql
-- Run LAST.
--
-- For every lead inserted by seed-03 (tagged raw_webhook_data->>'seed_batch'
-- = 'bulk_500'):
--   - 1 to 4 lead_interactions (call/whatsapp/email/in_person/sms, etc.)
--     with realistic notes and occurred_at timestamps after the lead's
--     created_at.
--   - 0 to 2 lead_follow_ups. Distribution:
...
--     NULL otherwise, satisfying check_follow_up_completion().
--
-- Note: marketing_leads_history and lead_assignment_log are NOT
-- hand-written here — they are populated automatically by the existing
-- audit_marketing_leads_changes / log_lead_assignment triggers whenever
...
BEGIN
  ...
  LOOP
    ...
    FOR v_k IN 1..v_num_ix LOOP
      ...
      SELECT id INTO v_itype_id FROM interaction_types WHERE name = v_itype_name;
...
BEGIN
  ...
  LOOP
    ...
    FOR v_k IN 1..v_num_fu LOOP
      ...
      END;
      SELECT id INTO v_status_id FROM follow_up_statuses WHERE name = v_status_name;
...
-- JOIN marketing_leads ml ON ml.id = li.lead_id
-- WHERE ml.raw_webhook_data->>'seed_batch' = 'bulk_500';
--
-- SELECT fs.name AS status, COUNT(*) FROM lead_follow_ups lf
